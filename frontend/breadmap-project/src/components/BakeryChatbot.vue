  ...
  <div class="fixed bottom-6 right-6 z-50">
    ...
    <Transition name="chat">
      ...
      >
        ...
        <!-- Step 0: API Key 입력 -->
        <div v-if="step === 'apikey'" class="flex-1 flex flex-col justify-center p-6 space-y-4 bg-gradient-to-b from-[#FFF9F0] to-white">
          <div class="text-center space-y-2">
            <div class="w-16 h-16 bg-[#FFF3DD] rounded-full flex items-center justify-center mx-auto mb-3">
              <span class="text-3xl">🔑</span>
            </div>
            <h2 class="text-xl font-jua text-[#6B4A38]">API 키 입력</h2>
            <p class="text-sm text-[#8B6A55]">SSAFY GMS API 키를 입력해주세요</p>
          </div>
          <input
            v-model="apiKey"
            type="password"
            placeholder="sk-..."
            class="w-full p-3 border-2 border-[#FFE8CC] rounded-xl focus:ring-2 focus:ring-[#F3B37A] focus:border-transparent outline-none transition font-jua"
          />
          <button
            @click="apiKey && (step = 'location')"
            :disabled="!apiKey"
            class="w-full py-3 rounded-xl font-jua text-white transition-all transform active:scale-95"
            :class="apiKey ? 'bg-[#F3B37A] hover:bg-[#C99768] shadow-md' : 'bg-gray-300 cursor-not-allowed'"
          >
            다음으로
          </button>
        </div>
    ...
    <button
      @click="toggleChat"
      class="w-16 h-16 bg-gradient-to-r from-[#F3B37A] to-[#C99768] rounded-full shadow-2xl flex items-center justify-center text-white hover:scale-110 transition-all duration-300 border-4 border-white"
...
const apiKey = ref('');
const step = ref('apikey'); // apikey -> location -> preference -> chat
const location = ref('');
...
const callOpenAI = async (currentMessages) => {
  ...
  try {
    ...
    messages.value.push({
      role: 'assistant',
      content: "죄송합니다. API 호출 중 오류가 발생했습니다. API 키를 확인하거나 네트워크를 확인해주세요."
    });
    scrollToBottom();
  } finally {
