  ...
  <div class="space-y-6 p-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        <span class="w-3 h-3 rounded-full bg-[#A1887F]"></span>
        <h2 class="text-2xl font-bold text-[#4E342E] font-serif">빵 주저리</h2>
      </div>
      <div class="flex items-center gap-2">
        ...
        <router-link :to="{ name: 'newChatter' }" class="px-5 py-2 text-sm font-bold rounded-full bg-[#8D6E63] text-white hover:bg-[#5D4037] transition-colors whitespace-nowrap shadow-md">
          글 작성 💬
        </router-link>
      </div>
    </div>
    ...
    <div class="space-y-4">
      <article
        ...
        :class="[
          ...
          selectedPostId === post.id ? 'ring-2 ring-[#D7CCC8] bg-[#FAFAFA]' : 'hover:border-[#BCAAA4]',
        ]"
      >
        <div class="flex items-center gap-2 mb-3">
          <span class="px-3 py-1 bg-[#F5F5F5] text-[#616161] text-xs font-bold rounded-full">
            빵 주저리
          </span>
        </div>
